...
program
    ...
    .action((options) => {
    render(React.createElement(App, { command: "review", options: options }));
});
program
    .command('add <files...>')
    ...
    render(React.createElement(App, { command: "add", files: files, options: options }));
});
program
    .command('stats')
    ...
    .action((options) => {
    render(React.createElement(App, { command: "stats", options: options }));
});
program
    .command('list')
    ...
    .action((options) => {
    render(React.createElement(App, { command: "list", options: options }));
});
program
    .command('sync <direction> <target>')
    .description('Sync challenges and progress')
    .action((direction, target, options) => {
    render(React.createElement(App, { command: "sync", direction: direction, target: target, options: options }));
});
program
    .command('config')
    ...
    .action((options) => {
    render(React.createElement(App, { command: "config", options: options }));
});
program.parse();
